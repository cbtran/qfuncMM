...
OptIntraOld::OptIntraOld(const arma::mat &data, const arma::mat &design,
                         const arma::mat &distSqrd, const arma::mat &timeSqrd,
                         int numVoxel, int numTimePt,
                         const arma::mat &fixedEffect, KernelType kernelType)
    : data_(data),
      design_(design),
      distSqrd_(distSqrd),
      timeSqrd_(timeSqrd),
...
double OptIntraOld::EvaluateWithGradient(const arma::mat &theta_unrestrict,
                                         arma::mat &gradient) {
  ...
  arma::mat dB_dk_gamma =
      logistic(theta_unrestrict(2)) * rbf(timeSqrd_, tau_gamma);
  arma::mat dB_dtau_gamma =
      k_gamma *
      (logistic(theta_unrestrict(1)) * rbf_deriv(timeSqrd_, tau_gamma)) %
      B_Region;
  ...
  arma::mat dC_dphi_gamma =
      logistic(theta_unrestrict(0)) *
      get_cor_mat_deriv(kernelType_, distSqrd_, phi_gamma);
  ...
  arma::vec VInv_r_region = kronecker_mvm(
      eigvec_C, eigvec_B,
      lambda_inv % kronecker_mvm(eigvec_C.t(), eigvec_B.t(), r_region));

  arma::mat VInv_Z_region = design_;
  VInv_Z_region.each_col([&eigvec_C, &eigvec_B, &lambda_inv](arma::vec &a) {
    a = kronecker_mvm(
        eigvec_C, eigvec_B,
        lambda_inv % kronecker_mvm(eigvec_C.t(), eigvec_B.t(), a));
  });
  ...
  arma::mat dV_phi_gamma_Z =
      kronecker_mmm(dC_dphi_gamma, B_Region, VInv_Z_region);
  arma::mat dV_tau_gamma_Z =
      kronecker_mmm(C_Region, dB_dtau_gamma, VInv_Z_region);
  ...
  arma::vec comp2(3);
  arma::vec dC_phi_temp =
      arma::kron(arma::diagvec(eigvec_C.t() * dC_dphi_gamma * eigvec_C),
                 arma::diagvec(eigvec_B.t() * B_Region * eigvec_B));
  comp2(0) = arma::sum(lambda_inv % dC_phi_temp);

  arma::vec dB_tau_temp =
      arma::kron(arma::diagvec(eigvec_C.t() * C_Region * eigvec_C),
  ...
  // w.r.t. theta.
  arma::vec comp3_1(3);
  arma::mat qdr_phi_gamma =
      -VInv_r_region.t() *
      kronecker_mvm(dC_dphi_gamma, B_Region, VInv_r_region);
  arma::mat qdr_tau_gamma =
      -VInv_r_region.t() *
      kronecker_mvm(C_Region, dB_dtau_gamma, VInv_r_region);
  arma::mat qdr_k_gamma =
      -VInv_r_region.t() * kronecker_mvm(C_Region, dB_dk_gamma, VInv_r_region);
