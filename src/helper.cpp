...
Rcpp::List asymptotic_variance(const arma::mat &SingleRegionMatrix_1,
  ...
  arma::mat V =
      arma::join_vert(
          arma::join_horiz(SingleRegionMatrix_1 + kron(J_L1, At), rhoL_At),
          arma::join_horiz(rhoL_At.t(),
                           SingleRegionMatrix_2 + kron(J_L2, At))) +
      I;
  ...
  arma::mat dV_rho =
      arma::join_vert(arma::join_horiz(Zero_L1_M, rhoLSqrd_At),
                      arma::join_horiz(rhoLSqrd_At.t(), Zero_L2_M));

  double tr = trace(
      0.5 * arma::powmat((VInv - VInv_Z * HInv * VInv_Z.t()) * dV_rho, 2));
