...
init_region <- function(region_info) {
  ...
  if (region_info$cov_setting == "noisy") {
    xi_ot <- s1$sigma2_ep * (xi_ot + 1)
    xi_im <- s1$sigma2_ep * xi_im
  }
  ...
  for (i in 1:10) {
    start_point <- stats::runif(2, min = 0.01, max = 10)
    optim_result <- stats::optim(start_point, xi_tau_opt,
  ...
  k_eta <- chi_im
  if (region_info$cov_setting == "noisy") {
    k_eta <- k_eta / s1$sigma2_ep
  }
  nugget_eta <- max(0.0001, chi_ot / chi_im - 1)
